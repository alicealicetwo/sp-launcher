...
This is currently a loadable shell with **no gameplay hooks**. When loaded, it
opens a console titled **SP Client Fixes** and prints a confirmation message;
the window closes with the game. Add the preservation fixes here after
translating and verifying each build-specific finding. The DLL must verify the
shipping executable's SHA-256
